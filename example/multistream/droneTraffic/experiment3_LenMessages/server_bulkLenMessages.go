...
func (t *trace) PrintDrone(tx_time int64, n int) {
	fmt.Fprintf(t.file, "%d\t%d\n", tx_time, n)
}

func streamCreator(sess quic.Connection, fileNameBulk string) int {
	...
	if id != 0{
		...
		i:=0
		//No priority stream
		buf := make([]byte, 18832) // Buffer for each stream
		for {
			if n, err := io.ReadFull(stream,buf); err != nil {
				fmt.Fprintln(outfile,"Error stream 0: ",err, time.Now())
				break
			} else {
				timeStamp = time.Now().UnixNano()
				t.PrintDrone(timeStamp, n)
				fmt.Fprintln(outfile,"TS of BULK at the server side\n", time.Now())
				bytesReceived += n
			}
		}
		fmt.Fprintln(outfile,"Server - Number of bytes received by the stream ",stream.StreamID(),":", bytesReceived," e i:",i)
		totalBytesReceived=bytesReceived
...
func main() {
	...
	for i := 0; i < numThreads; i++ {
		go func() {
			defer wg.Done()
			n:= streamCreator(sess, *fileNameBulk)
			mutex.Lock()
			maxBytes += n
			mutex.Unlock()
		}()
