...
func streamCreator(sess quic.Connection, fileName string) int {
	...
	// As Pablo did...
	receiveAsMuchAsPossible := func(conn quic.Stream, streamID quic.StreamID) {
		for {
			if conn == nil {
...
func main() {
	...
	go func() {
		...
		for _, dLen := range dataLen {
			//if err := stream.SetReadDeadline(time.Now().Add(3*time.Millisecond)); err != nil {
			//	fmt.Println("Could not set connection read deadline: " + err.Error())
			//}

			if n, err := stream.Read(buf[:dLen]); err != nil {
				break
			} else {
				bytesReceived += n
